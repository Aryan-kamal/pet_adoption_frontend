...
function Dog() {
	...
	useEffect(() => {
		const fetchDogs = async () => {
			try {
				const res = await axios.get(`${serverUrl}/pets?category=Dog`);
				setDogs(res.data);
		...
		<>
			...
			<div className="container mt-5">
				...
				<div className="mb-4">
					<button
						className={`btn me-2 ${
							filter === "Special" ? "btn-primary" : "btn-outline-primary"
						}`}
					...
					<button
						className={`btn me-2 ${
							filter === "Normal" ? "btn-secondary" : "btn-outline-secondary"
						}`}
					...
					<button
						className={`btn me-2 ${
							filter === "puppy" ? "btn-secondary" : "btn-outline-third"
						}`}
